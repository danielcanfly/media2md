# Media2MD v0.7.9 Strict Acceptance
...
| 31 | Installer verifies actual installed version | PASS | Installer clears stale bytecode and verifies package version, project script version, and final `media2md 0.7.9`. |
...
## Required live rerun before v0.7.9 can be called functionally complete

1. Upgrade and confirm package, script, and project CLI all report `0.7.9`.
2. Resume `@startupbell` from item 476.
3. Confirm the first attempt prints either `SYNC_TRANSPORT_HINT_LOADED` or `SYNC_TRANSPORT_HINT_SELECTED ... direct-plain` for an old checkpoint under macOS proxy.
4. Confirm the next successful page prints `SYNC_TRANSPORT_HINT_SAVED`.
5. When stable-ID TLS failures open the circuit, confirm switching to the handle candidate does not restart configured/latest browser impersonation.
6. Confirm no attempt starts with `page_budget_remaining_seconds` below five.
7. Confirm a four-page pause reports `reason=max_pages_per_run`.
8. Confirm the JSON summary includes `creator_identifiers.sec_uid`.
9. Continue bounded invocations until a final page smaller than 25 establishes the exact TikTok total.
10. Complete the remaining YouTube file/frontmatter, auth Doctor, and OpenClaw cron live checks.
