...
def backup(target:Path)->Path|None:
 ...
 archive=root/f'rollback-before-v084-{stamp()}.zip'
...
def migrate_tiktok_cursor_state(target: Path) -> tuple[int,int]:
 ...
 for path in checkpoint_dir.glob('tiktok-*.json'):
  ...
  if device_id:
   ...
   migrated_devices += 1
  if handle.lower() in exact_handles and not payload.get('items'):
