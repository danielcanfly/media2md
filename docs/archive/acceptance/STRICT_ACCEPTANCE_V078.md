# Media2MD v0.7.8 Strict Acceptance
...
| 31 | Installer verifies actual installed version | PASS | Installer clears stale bytecode and verifies package version, project script version, and final `media2md 0.7.8`. |
...
| 82 | Budget exhaustion pauses instead of failing or running forever | FIXED-LIVE | Live v0.7.7 preserved the checkpoint but still returned `ERROR`. v0.7.8 converts bounded-page exhaustion into `SYNC_RUN_PAUSED reason=page_budget_exhausted`. |
...
| 84 | TikTok run progress is visible across pages | LIVE | The event exists and passed automated tests; the v0.7.7 fixture page did not complete, so a live `SYNC_RUN_PROGRESS` line is still required. |

| 85 | Stable-ID catalog payload may reuse the already-known human handle | FIXED-LIVE | v0.7.7 transport successfully returned page JSON but rejected it because `tiktokuser:<secUid>` omitted `@startupbell`. v0.7.8 supplies the normalized creator handle as metadata context. |
| 86 | Stable-ID success must not trigger a second handle-profile fetch | FIXED-LIVE | On success v0.7.8 emits `SYNC_STABLE_ID_HANDLE_REUSED ... second_profile_fetch=false` and completes the page immediately. |
| 87 | Page-budget exhaustion returns a resumable partial result, not a hard error | FIXED-LIVE | v0.7.8 returns `SYNC_RUN_PAUSED reason=page_budget_exhausted`, preserves exact=false and resumes from the same item. |
...
## Required live rerun before v0.7.8 can be called functionally complete

1. Upgrade and confirm package, script, and project CLI all report `0.7.8`.
2. Resume `@startupbell` from item 376 with the same page size and budgets.
3. Confirm the first successful stable-ID transport prints `SYNC_STABLE_ID_HANDLE_REUSED ... second_profile_fetch=false`, then immediately prints `SYNC_PAGE_DONE` without a handle candidate.
4. Confirm a real page-budget exhaustion prints `SYNC_RUN_PAUSED reason=page_budget_exhausted`, returns to the shell without `ERROR`, and preserves `resume_from`.
5. Confirm successful pages print `SYNC_RUN_PROGRESS` and the next page prefers the last successful transport.
6. Continue bounded invocations until a final page smaller than 25 establishes the exact TikTok total.
7. Confirm the specified long YouTube video creates multiple chunks and one merged Markdown artifact.
8. Confirm a single Short lands under `shorts/`, contains `media_type: youtube_short`, and links to the existing creator.
9. Complete Instagram/TikTok auth verify and Doctor tests, then a real OpenClaw cron trigger.
