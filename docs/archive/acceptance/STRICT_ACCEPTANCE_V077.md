# Media2MD v0.7.7 Strict Acceptance
...
| 31 | Installer verifies actual installed version | PASS | Installer clears stale bytecode and verifies package version, project script version, and final `media2md 0.7.7`. |
...
| 47 | No silent infinite retry | FIXED-LIVE | v0.7.6 had finite child attempts but no page-level or invocation-level deadline, so total runtime multiplied across candidates and pages. v0.7.7 adds both hard bounds and resumable pause. |
...
| 52 | TikTok quiet-period telemetry is understandable | FIXED-LIVE | `SYNC_HEARTBEAT` is removed. v0.7.7 emits `SYNC_WAITING` every 30 seconds with range, candidate, strategy, auth mode, and elapsed time. |
| 53 | TikTok page and extractor attempts have finite deadlines | FIXED-LIVE | Per-attempt timeout remains configurable, while one shared `MEDIA2MD_TIKTOK_PAGE_BUDGET_SECONDS` now caps all candidates and strategies for a page. |
...
| 62 | Instagram cookie path preserves the proven default | FIXED-LIVE | v0.7.7 reverses the brittle forced argument: the worker resolves the managed cookie file exactly as v0.6.x did. Explicit `--cookies-file` remains optional. One real Reel must complete. |
...
| 77 | One shared deadline per TikTok page | FIXED-LIVE | All stable-ID, handle, transport, and auth attempts share one deadline. Fallback cannot restart the page clock. Live output must show `SYNC_PAGE_BUDGET`. |
...
| 79 | Last successful TikTok strategy is reused first | FIXED-LIVE | The successful transport/auth pair is cached within the sync process and reordered to the front on the next page. Live evidence required. |
...
| 82 | Budget exhaustion pauses instead of failing or running forever | FIXED-LIVE | `SYNC_RUN_PAUSED` returns an exact-false partial summary with `resume_from`; rerunning the same command continues from the checkpoint. |
| 83 | TikTok attempt outcomes are explicit | FIXED-LIVE | Every transport attempt now ends with `SYNC_ATTEMPT_RESULT status=success|failed reason=... elapsed_seconds=...`. |
| 84 | TikTok run progress is visible across pages | FIXED-LIVE | `SYNC_RUN_PROGRESS` reports pages completed, known items, next start, and total elapsed time, preventing per-process counters from masquerading as overall progress. |

## Required live rerun before v0.7.7 can be called functionally complete

1. Upgrade and confirm package, script, and project CLI all report `0.7.7`.
2. Resume `@startupbell` with page size 25, page budget 300 seconds, total runtime 1800 seconds, and maximum four pages per invocation.
3. Confirm one page prints only one `SYNC_PAGE_BUDGET`, no nested `SYNC_NETWORK_CONTEXT` ladder, and contextual `SYNC_WAITING` rather than restarting `SYNC_HEARTBEAT` counters.
4. Confirm successful pages print `SYNC_ATTEMPT_RESULT status=success`, `SYNC_PAGE_DONE`, and `SYNC_RUN_PROGRESS`.
5. Confirm a four-page or runtime limit prints `SYNC_RUN_PAUSED`, returns to the shell, preserves the checkpoint, and resumes from `resume_from` on the next invocation.
6. Continue bounded invocations until a final page smaller than 25 establishes the exact TikTok total.
7. Press Ctrl+C during one attempt and confirm exit code 130 with checkpoint intact.
8. Confirm the specified long YouTube video creates multiple chunks and one merged Markdown artifact.
9. Confirm a single Short lands under `shorts/`, contains `media_type: youtube_short`, and links to the existing creator.
10. Complete Instagram/TikTok auth verify and Doctor tests, then a real OpenClaw cron trigger.
