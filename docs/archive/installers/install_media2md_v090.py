...
def backup(target:Path)->Path|None:
 ...
 archive=root/f'rollback-before-v090-{stamp()}.zip'
...
def migrate_tiktok_cursor_state(target: Path) -> tuple[int,int]:
 checkpoint_dir=target/'data'/'provider_catalog_checkpoints'
 db=target/'data'/'media2md.db'
 if not checkpoint_dir.is_dir(): return (0,0)
 exact_handles=set()
 if db.is_file():
  conn=sqlite3.connect(db)
  try:
   columns={row[1] for row in conn.execute("PRAGMA table_info(creators)").fetchall()}
   if {'provider','handle','current_total_exact'}.issubset(columns):
    exact_handles={str(row[0]).lower() for row in conn.execute(
     "SELECT handle FROM creators WHERE provider='tiktok' AND current_total_exact=1"
    ).fetchall()}
  finally:
   conn.close()
 state_path=checkpoint_dir/'tiktok-cursor-state.json'
 try: state=json.loads(state_path.read_text(encoding='utf-8'))
 except (FileNotFoundError,json.JSONDecodeError,OSError): state={'schema_version':1,'creators':{}}
 state.setdefault('schema_version',1); creators=state.setdefault('creators',{})
 migrated_devices=0; staged_exact=0
 for path in checkpoint_dir.glob('tiktok-*.json'):
  if path.name=='tiktok-cursor-state.json': continue
  try: payload=json.loads(path.read_text(encoding='utf-8'))
  except (json.JSONDecodeError,OSError): continue
  handle=str(payload.get('creator') or path.stem.removeprefix('tiktok-')).lstrip('@')
  device_id=str(payload.get('tiktok_device_id') or '').strip()
  if device_id:
   creators[handle]={'device_id':device_id,'authenticated':bool(payload.get('preferred_authenticated',False)),'updated_at':datetime.now(timezone.utc).isoformat(timespec='seconds')}
   migrated_devices += 1
  if (
   handle.lower() in exact_handles
   and payload.get('mode') == 'full'
   and payload.get('pagination_backend') == 'cursor_api'
  ):
   if not payload.get('rebuild_from_exact'):
    payload['rebuild_from_exact']=True
 ...
 return migrated_devices,staged_exact
...
def main():
 ...
 migrated_cursor_devices,staged_exact_rebuilds=migrate_tiktok_cursor_state(target)
