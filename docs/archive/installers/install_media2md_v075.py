...
def backup(target:Path)->Path|None:
 ...
 archive=root/f'rollback-before-v075-{stamp()}.zip'
...
def main():
 ...
 print('MEDIA2MD_V075_INSTALLED'); print(f'version={VERSION}'); print(f'project_root={target}'); print(f'rollback_backup={rollback}'); print(f'pip_exit_code={pip_exit}'); print(f'bytecode_caches_removed={purged_before+purged_after_copy+(purged_after_pip if not a.skip_pip else 0)}'); print(f'instagram_contract_regression_items_requeued={repaired_instagram_items}'); print('data_preserved=config,data,logs,workspace,markdown,downloads,transcripts'); return 0
